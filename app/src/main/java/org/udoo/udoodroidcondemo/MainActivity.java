...
//	    debug_tv.setVisibility(View.GONE);
	    ...
	    voiceButton.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				tts.speak("Good morning sir, would you like a coffee?", TextToSpeech.QUEUE_FLUSH, null);
...
//	                    tts.setPitch(0.7F);
    ...
    private boolean searchCommands (String result) {   	
        ...
        for (String string : allwords ) {
        	if (result.toLowerCase(Locale.US).contains(string.toLowerCase(Locale.US))) {
    	        ...
    	        else if (forward_strings.contains(stringFounded)) mAdkManager.write(ESPRESSO_SENDSTRING);
    	        else if (backward_strings.contains(stringFounded)) mAdkManager.write(LONG_SENDSTRING);
    	        else if (right_strings.contains(stringFounded)) mAdkManager.write(CAPPUCCINO_SENDSTRING);
    	        else if (left_strings.contains(stringFounded)) mAdkManager.write(CHOCO_SENDSTRING);
    ...
    private void moonWalkCase() {
		...
		mAdkManager.write(MOONWALK_SENDSTRING);
    ...
    private void sendHello() {
		...
		mAdkManager.write(HELLO_SENDSTRING);
    ...
    private void sendGoodCase() {
    	...
    	setNewFace(R.drawable.happy);
		goodEffect.play();
		mAdkManager.write(GOODBOY_SENDSTRING);
		returnToNormalState(5000);
    }
    ...
    private void sendBadCase() {
		...
		badEffect.play();
		mAdkManager.write(BADBOY_SENDSTRING);
		returnToNormalState(5000);
    }
    ...
    private void sendCuteCase() {
		...
		cuteEffect.play();
		mAdkManager.write(CUTEBOY_SENDSTRING);
		returnToNormalState(5000);
    }
